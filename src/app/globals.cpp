...
KonstruktorApplication::KonstruktorApplication(QObject *parent) : QObject(parent)
{
...
QString KonstruktorApplication::escape(const QString &str)
{
	QString r = str;
	r.replace('\'', "''");
	return r;
...
QString KonstruktorApplication::unescape(const QString &str)
{
	QString r = str;
	r.replace("''", "'");
	return r;
...
float KonstruktorApplication::floatModulo(float dividend, float divisor)
{
	if (dividend > 0.0f) {
